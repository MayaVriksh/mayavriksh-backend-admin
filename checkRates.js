...
const SHIPROCKET_API_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..UNWfUf7R2eHEr2_nomeJjbI_UIX28v7fKs8StXLC_B0';
...
async function checkShippingRates(destinationPincode) {
    ...
    try {
        ...
        if (data.status === 200 && data.data?.available_courier_companies) {
            ...
            const errorMessage = data.message || 'Pincode not serviceable or error occurred.';
            console.log(`   -> Info: ${errorMessage}`);
            return [];
        }
    } catch (error) {
        console.error(`   -> ❌ API Request Error for Pincode ${destinationPincode}:`, error.message);
        return null;
    }
}
...
async function main() {
    ...
    if (!fs.existsSync(jsonFilePath)) {
        console.error(`❌ Error: '${jsonFilePath}' file not found. Please create it first.`);
        return;
    }
    ...
    for (const pincode of destinationPincodes) {
        ...
        if (results && results.length > 0) {
            results.forEach(courier => {
                ...
                console.log(`   - 🚚 Courier: ${courierName.padEnd(15)} | 💰 Rate: ₹${rate.padEnd(7)} | 📅 ETD: ${etd}`);
            });
        }
        console.log("-".repeat(60));
    }
