...
const WarehouseService = require("../services/warehouse.service.js");
const uploadBufferToCloudinary = require("../../../../utils/mediaUpload.util.js");
const { getMediaType } = require("../../../../utils/file.utils.js");

const showWarehouseProfile = async (req, h) => {
    try {
        const { userId } = req.auth;

        const result = await WarehouseService.showWarehouseProfile(userId);
...
const completeSupplierProfile = async (req, h) => {
    try {
        ...
        // Pass all to service
        const result = await WarehouseService.completeSupplierProfile(
            userId,
            {
                ...profileFields,
                tradeLicenseUrl
...
const updateWarehouseProfile = async (req, h) => {
    try {
        const { userId } = req.auth;
        ...
        const result = await WarehouseService.updateWarehouseProfile(
            userId,
            updateData,
            profileImageUrl
        );
